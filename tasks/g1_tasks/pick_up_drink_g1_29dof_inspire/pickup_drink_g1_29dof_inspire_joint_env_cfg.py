...
"""Pick-up task for drink bottle with graduated reward for G1 + Inspire hand.

Uses 4-tier reward (reach/grasp/lift/place) instead of binary post-placement.
Scene uses a cylindrical drink bottle (~6cm diameter x 15cm tall, ~300g).
"""
import torch
...
from tasks.common_scene.base_scene_pickup_drink import TableDrinkSceneCfg


@configclass
class PickUpDrinkSceneCfg(TableDrinkSceneCfg):
    """Scene: table + drink bottle + G1 Inspire hand."""
...
@configclass
class PickUpDrinkG129InspireEnvCfg(ManagerBasedRLEnvCfg):
    """G1 drink bottle pick-up environment with graduated reward and Inspire hand."""

    scene: PickUpDrinkSceneCfg = PickUpDrinkSceneCfg(
        num_envs=1,
        env_spacing=2.5,

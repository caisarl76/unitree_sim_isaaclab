...
"""Pill bottle pick-and-place task with 5-tier graduated reward for G1 + Inspire hand.

Uses pill bottle scene (cylinder object + red tape target).
Monocular D435i camera for training data compatibility.
"""
import torch
...
@configclass
class PillBottleSceneCfg(TablePillBottleSceneCfg):
    """Pill bottle scene with G1 Inspire hand and D435i camera."""
...
@configclass
class PickPlacePillBottleG129InspireEnvCfg(ManagerBasedRLEnvCfg):
    """G1 pill bottle pick-and-place environment with 5-tier reward."""

    scene: PillBottleSceneCfg = PillBottleSceneCfg(
        num_envs=1,
        env_spacing=2.5,
